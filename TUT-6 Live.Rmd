...
- What to include? \
  From the test: \
    $H_{0}:$ There is _______ in mean score between the 2 courses $\mu  0$ \
    $H_{a}:$ There is _______ in mean score between the 2 courses $\mu  0$ \
  From the results: \
    p-value = _____ which is  $\alpha = 0.05$. \
    Hence we **__________ the $H_{0}$ ** at $\alpha$ level of significance meaning there ________________   mean math phobia scores are different between the two courses. \
...
```
  - Boxplot `a` -> _____________ .\
  - Boxplot b ->  ___________. \
  - Median(2Q) _____________ \
...
```
- 1 \
t.test relies on ________, 

`a` ____ symetric at all `b` is ____ symetric BUT !!.
Also recall what we're testing in a t.test? \
$\mu$ what happens to the sensitivity of mean when data is limited 
...
ggplot(carbon, aes(x = factor(method), y = weight)) +
  geom_*****() #boxplot OR side-by-sideHistogram
```
  # Median Comparison \
  - . \ 
  # Spread comparison \
